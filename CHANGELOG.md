...
### Added
- The session id in the card view header is now editable (pencil icon): paste the right conversation UUID if the wrong one got linked, or clear it to start the next session fresh. The edit syncs to the running session, and auto-detection won't immediately override it — only conversation files with activity after the edit may take over.
